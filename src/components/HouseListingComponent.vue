<template>
  <article class="house-card" @click="goToDetails">
      ...
      <h2 class="house-card-title">{{ house.location.street }} {{ house.location.houseNumber }} {{ house.location?.houseNumberAddition }}
        <div class="house-card-actions" v-if="house.madeByMe">
          <router-link
            ...
            class="btn btn-edit" @click.stop>
              <img class="btn-edit-icon" src="../assets/ic_edit@3x.png" alt="Edit" />
          </router-link>
          <button class="btn btn-delete" type="button" @click.stop="$emit('request-delete', house)">
...
<script>
  ...
  export default {
    ...
    setup(props) {
      ...
      const goToDetails = () => {
        router.push({ name: 'HouseDetailsView', params: { id: props.house.id } })
      }
